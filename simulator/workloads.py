...
def all_to_all_workload(gpus: list[GPU], base_rate: float = 4.0) -> list[Flow]:
    ...
    per_flow_rate = base_rate / num_peers
    for src in gpus:
        for dst in gpus:
...
def web_traffic_workload(gpus: list[GPU], base_rate: float = 2.0, density: float = 0.2) -> list[Flow]:
    ...
    for src in gpus:
        ...
        per_flow_rate = base_rate / len(active_dsts)
